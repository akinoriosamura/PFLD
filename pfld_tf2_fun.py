...
from tensorflow.keras.layers import Input, Add, Flatten, Concatenate, Dense, Conv2D, DepthwiseConv2D, MaxPooling2D, AveragePooling2D, GlobalAveragePooling2D, BatchNormalization
...
class PFLDBackbone(Model):
    ...
    """
    def Bottleneck(self, is_first, s, channel, in_channel, up_sample, name):
        tchannel = in_channel * up_sample

    def call(self, inputs):
        x = self.conv2d(self.tchannel, 1, 1, padding='same', name=self.vname+'_conv2d_1')(inputs)
        x = BatchNormalization(name=self.vname+'_bn1')(x)# , training=training)
        x = tf.nn.relu(x)
        x = DepthwiseConv2D(3, strides=(self.s, self.s), depth_multiplier=1, padding='same', name=self.vname+'_dwconv2d_1')(x)
        x = BatchNormalization(name=self.vname+'_bn2')(x)# , training=training)
        x = tf.nn.relu(x)
        x = self.conv2d(self.channel, 1, 1, padding='same', name=self.vname+'_conv2d_3')(x)
        x = BatchNormalization(name=self.vname+'_bn3')(x)# , training=training)
        if (self.s == 1) and (self.channel == self.in_channel):
            x = Add()([x, inputs])
    ...
    def create_model(self, args, in_shape):
        print("=== start network ===")
        # import pdb;pdb.set_trace()
        print('PFLD input shape: {}'.format(in_shape))
        ...
        model = tf.keras.Model(inputs=input, outputs=[features3, landmarks], name='pfld_backbone_model')

        # divide by 2 since d/dweight learning_rate * weight^2 = 2 * learning_rate * weight
        # see https://arxiv.org/pdf/1711.05101.pdf
        regularizer = tf.keras.regularizers.l2(args.weight_decay)
...
class PFLDAuxiliary(Model):
    ...
    def create_model(self, args, in_shape):
        ...
        model = tf.keras.Model(inputs=input, outputs=euler_angles_pre, name='pfld_aux_model')
        ...
        model.summary()
        keras.utils.plot_model(model, 'my_first_pfldaux_with_shape_info.png', show_shapes=True)
