...
def pfld_backbone(input, weight_decay, batch_norm_params, num_labels, depth_multi, min_depth=8):
    ...
    with tf.variable_scope('pfld_inference'):
        ...
        with slim.arg_scope(
            ...
            # no depth for match pre trained model
            num_channel = 32
            conv7 = conv2d(conv6, stride=2, channel=32,
                           kernel=3, depth=depth, scope='conv7')
            # 7*7*32 / conv7*7 / c:128,n:1,s:1
            num_channel = depth(128)
            # conv8 = slim.conv2d(conv7, num_channel, [7, 7], stride=1, padding='VALID', scope='conv8')
            # for img size84
            conv8 = slim.conv2d(conv7, num_channel, [
                                5, 5], stride=1, padding='VALID', scope='conv8')
...
def create_model(input, landmark, phase_train, args):
    ...
    time.sleep(3)
    features, landmarks_pre = pfld_backbone(
