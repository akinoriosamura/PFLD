...
class PnpHeadPoseEstimator:
    ...
    def _return_landmarks(self, inputImg, points_to_return=range(0, 68)):
        ...
        try:
            ...
            det = dlib.rectangle(
                left=0, top=0, right=int(img_w), bottom=int(img_h))
            return None
        ...
        for point in points_to_return:
            landmarks[counter] = [dlib_landmarks.parts(
            )[point].x, dlib_landmarks.parts()[point].y]
            counter += 1
    ...
    def return_pitch_yaw_roll(self, image, radians=False):
        ...
        landmarks_2D = self._return_landmarks(
            inputImg=image, points_to_return=TRACKED_POINTS)
...
if __name__ == '__main__':
    ...
    while(cap.isOpened()):
        ...
        if pitch_yaw_roll is not None:
            ...
            cv2.putText(frame, 'pitch:{:+.2f}'.format(pitch),
                        (0, 20), cv2.FONT_HERSHEY_PLAIN, 1, (0, 0, 255), 1)
            cv2.putText(frame, 'yaw:{:+.2f}'.format(yaw), (0, 35),
                        cv2.FONT_HERSHEY_PLAIN, 1, (0, 0, 255), 1)
            cv2.putText(frame, 'roll:{:+.2f}'.format(roll),
