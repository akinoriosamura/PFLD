...
def XinNingNetwork1(input, is_training, weight_decay, batch_norm_params, num_labels, depth_multi, min_depth=8):
    ...
    with tf.variable_scope('pfld_inference1'):
        ...
        with slim.arg_scope(
            ...
            conv1_1 = conv2d(input, stride=2, channel=16,
                             kernel=3, depth=depth, scope='conv1_1')
            print(conv1_1.name, conv1_1.get_shape())
            # 56*56*16 / conv3*3 / c:32,n:1,s:2
            conv1_2 = conv2d(conv1_1, stride=2, channel=32,
                             kernel=3, depth=depth, scope='conv1_2')
            print(conv1_2.name, conv1_2.get_shape())
            # 28*28*32 / pool2*2 / c:32,n:1,s:2
            pool1_2 = slim.max_pool2d(conv1_2, kernel_size=[
                                      2, 2], stride=2, scope='pool1_2', padding='SAME')
            print(pool1_2.name, pool1_2.get_shape())
            # 14*14*32 / conv3*3 / c:64,n:1,s:2
            conv1_2_1 = conv2d(pool1_2, stride=2, channel=64,
                               kernel=3, depth=depth, scope='conv1_2.1')
            print(conv1_2_1.name, conv1_2_1.get_shape())
            # 7*7*64 / global_pool / c:64,n:1
            pool1_2_1 = slim.avg_pool2d(conv1_2_1, [7, 7], stride=[
                                        7, 7], scope='pool1_2.1', padding='SAME')
            print(pool1_2_1.name, pool1_2_1.get_shape())
            # 14*14*32 / conv3*3 / c:64,n:1,s:2
            conv1_3 = conv2d(pool1_2, stride=2, channel=64,
                             kernel=3, depth=depth, scope='conv1_3')
            print(conv1_3.name, conv1_3.get_shape())
            # 7*7*64 / pool2*2 / c:64,n:1,s:2
            pool1_3 = slim.max_pool2d(conv1_3, kernel_size=[
                                      2, 2], stride=2, scope='pool1_3', padding='SAME')
            print(pool1_3.name, pool1_3.get_shape())
            # 4*4*64 / conv3*3 / c:64,n:1,s:2
            conv1_3_1 = conv2d(pool1_3, stride=2, channel=64,
                               kernel=3, depth=depth, scope='conv1_3.1')
            print(conv1_3_1.name, conv1_3_1.get_shape())
            # 2*2*64 / global_pool / c:64,n:1
            pool1_3_1 = slim.avg_pool2d(conv1_3_1, [2, 2], stride=[
                                        2, 2], scope='pool1_3.1', padding='SAME')
            print(pool1_3_1.name, pool1_3_1.get_shape())
            # 4*4*64 / conv3*3 / c:64,n:1,s:2
            conv1_4 = conv2d(pool1_3, stride=2, channel=64,
                             kernel=3, depth=depth, scope='conv1_4')
            print(conv1_4.name, conv1_4.get_shape())
            # 2*2*64 / global_pool / c:64,n:1
            pool1_4_1 = slim.avg_pool2d(conv1_4, [2, 2], stride=[
                                        2, 2], scope='pool1_4.1', padding='SAME')
            ...
            output_1 = slim.fully_connected(
                flattened_1, num_outputs=num_labels*2, scope='fc_1')
...
def XinNingNetwork2(input, heatmap, is_training, weight_decay, batch_norm_params, num_labels, depth_multi, min_depth=8):
    ...
    with tf.variable_scope('pfld_inference2'):
        ...
        with slim.arg_scope(
            ...
            conv2_1 = conv2d(concatted_2, stride=2, channel=8,
                             kernel=3, depth=depth, scope='conv2_1')
            print(conv2_1.name, conv2_1.get_shape())
            # 56*56*8 / pool3*3 / c:28,n:1,s:2
            pool2_1 = slim.max_pool2d(conv2_1, kernel_size=[
                                      3, 3], stride=2, scope='pool2_1', padding='SAME')
            print(pool2_1.name, pool2_1.get_shape())
            # 28*28*8 / conv3*3 / c:16,n:1,s:1
            conv2_2 = conv2d(pool2_1, stride=1, channel=16,
                             kernel=3, depth=depth, scope='conv2_2')
            print(conv2_2.name, conv2_2.get_shape())
            # 28*28*16 / pool3*3 / c:16,n:1,s:2
            pool2_2 = slim.max_pool2d(conv2_2, kernel_size=[
                                      3, 3], stride=2, scope='pool2_2', padding='SAME')
            print(pool2_2.name, pool2_2.get_shape())
            # 14*14*16 / conv3*3 / c:64,n:1,s:1
            conv2_2_1 = conv2d(pool2_2, stride=1, channel=64,
                               kernel=3, depth=depth, scope='conv2_2.1')
            print(conv2_2_1.name, conv2_2_1.get_shape())
            # 14*14*64 / global_pool / c:64,n:1
            pool2_2_1 = slim.avg_pool2d(conv2_2_1, [14, 14], stride=[
                                        14, 14], scope='pool2_2.1', padding='SAME')
            print(pool2_2_1.name, pool2_2_1.get_shape())
            # 14*14*16 / conv3*3 / c:64,n:1,s:2
            conv2_3 = conv2d(pool2_2, stride=2, channel=64,
                             kernel=3, depth=depth, scope='conv2_3')
            print(conv2_3.name, conv2_3.get_shape())
            # 7*7*64 / pool3*3 / c:64,n:1,s:2
            pool2_3 = slim.max_pool2d(conv2_3, kernel_size=[
                                      3, 3], stride=2, scope='pool2_3', padding='SAME')
            print(pool2_3.name, pool2_3.get_shape())
            # 3*3*64 / conv3*3 / c:64,n:1,s:1
            conv2_3_1 = conv2d(pool2_3, stride=2, channel=64,
                               kernel=3, depth=depth, scope='conv2_3.1')
            print(conv2_3_1.name, conv2_3_1.get_shape())
            # 3*3*64 / global_pool / c:64,n:1
            pool2_3_1 = slim.avg_pool2d(conv2_3_1, [3, 3], stride=[
                                        3, 3], scope='pool2_3.1', padding='SAME')
            print(pool2_3_1.name, pool2_3_1.get_shape())
            # 3*3*64 / conv3*3 / c:64,n:1,s:2
            conv2_4 = conv2d(pool2_3, stride=2, channel=64,
                             kernel=3, depth=depth, scope='conv2_4')
            print(conv2_4.name, conv2_4.get_shape())
            # 2*2*64 / global_pool / c:64,n:1
            pool2_4_1 = slim.avg_pool2d(conv2_4, [2, 2], stride=[
                                        2, 2], scope='pool2_4.1', padding='SAME')
            ...
            output_2 = slim.fully_connected(
                flattened, num_outputs=num_labels*2, scope='fc_2')
...
def create_model(input, landmark, phase_train, args, mean_shape=None):
    ...
    _, heatmap, _heat_values = XinNingNetwork1(
        input, stage1_training, args.weight_decay, batch_norm_params, args.num_labels, args.depth_multi)
    landmarks_out = XinNingNetwork2(input, heatmap, stage2_training,
                                    args.weight_decay, batch_norm_params, args.num_labels, args.depth_multi)
    # loss
